...
<div id="content" class="form_divdrop-shadow ">
    ...
    <div class="form_div drop-shadow border-all center-block">
            ...
            @endif
            <form role="form" id="newsletter_signup">              
                <div class="form-group">
                    {{ Form::label('fname', "First Name",["class"=>"text-muted"]) }}
                    {{ Form::text('fname',Input::old('fname'),["class"=>"form-control","placeholder"=>"Enter First Name", 'required' => 'required']) }}
                    {{ $errors->first('fname', '<span class=warning>:message</span>') }}
                </div>             
                <div class="form-group">
                    {{ Form::label('lname', "Last Name",["class"=>"text-muted"]) }}
                    {{ Form::text('lname',Input::old('lname'),["class"=>"form-control","placeholder"=>"Enter Last Name", 'required' => 'required']) }}
                    {{ $errors->first('lname', '<span class=warning>:message</span>') }}
                </div>
                <div class="form-group">	
                    {{ Form::label('email', "Email",["class"=>"text-muted"]) }}
                    {{ Form::email('email',Input::old('email'),["class"=>"form-control","placeholder"=>"Enter Email", 'required' => 'required']) }}
